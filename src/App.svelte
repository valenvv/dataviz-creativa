<script>
  ...
  //////////  Color de la flor  -> Género literario a seleccionar
  ...
  function iniciarFiltro(filter, value) {
    if (filter === "genero") {
    ...
    // Verificar si ambos filtros están vacíos y actualizar los datos filtrados
  ...
  function filtrarDatos() {
    ...
    let datosFiltrados = datos; // Inicialmente se muestran todos los datos
  ...
  //Actualización dinámica del filtro
...
<main>
  ...
  <div class="filtro">
    <select bind:value={filtroGenero} on:change={(e) => iniciarFiltro(filtroGenero, e.target.value)}>
  ...
  <select bind:value={filtroMotivacion} on:change={(e) => iniciarFiltro(filtroMotivacion, e.target.value)}>
  ...
  <div class="container">
    {#each datosFiltrados as item}
      <div class="elemento">
        ...
        <!-- SVG de distinto tipo de flor, dependiendo Motivación para leer -->
        ...
        <!-- Tallo de la flor -->
        <div class="tallo">
          <object class="tallo" data="{obtenerSVGTallo(item.Horas)}" type="image/svg+xml" height="90" aria-label="Tallo SVG"></object>
        </div>

        <!-- Libro favorito de la persona -->
    ...
    <div class="r-tallo">
      ...
      <ul>
        {#each Object.keys(svgsTallo) as svg}
          <li><img src="{obtenerSVGTallo(svg)}" alt="{svg}" height="100">{svg}</li>
        {/each} 
      </ul>
    </div>
    <div class="r-centros">
